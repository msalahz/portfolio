...
- `src/features/landing/components/WorkHistorySection.tsx`
- `src/features/landing/components/StatsSections.tsx`
- `src/features/landing/components/ContactSection.tsx`
- `src/features/landing/components/HeroCompanies.tsx`
...
4. **Heading Hierarchy:** Sections use `h2` for titles, often with `text-4xl font-medium text-balance lg:text-5xl`
...
- **`useInView`** hook (`src/core/hooks/useInView.ts`)
...
Located in `src/styles.css` (lines 22-91).
...
Located in `src/integrations/shadcn/components/ui/`.
...
1. **`AnimatedNum`** (`src/core/components/AnimatedNum.tsx`)
...
2. **`Marquee`** (`src/core/components/Marquee.tsx`)
   ...
   - Used in `HeroCompanies` section with `[--duration:20s]`

### Theme Components

- `ThemeProvider`, `ThemeToggle`, `useTheme` for dark mode support

### Utilities

- `cn()` from `shadcn/lib/utils` - class merging utility (clsx + tailwind-merge)
...
- `HeroHeadshot` uses radial mask utilities:
  ```
  mask-y-from-85% mask-y-to-95% mask-x-from-85% mask-x-to-95% mask-radial-from-50% mask-radial-to-55%
  ```
...
- **Muted section background:** `dark:bg-background bg-muted/50`
...
- **Bundler:** Vite with TailwindCSS v4.1 via `@tailwindcss/vite` plugin
- **Framework:** TanStack Start
- **Config:** Tailwind configured via `vite.config.ts` (line 24: `tailwindcss()`)
...
All animations are performant, using CSS where possible and React state minimally. The design is highly componentized with consistent spacing (4/6/8/12/16/32px scale), and a clean color palette supporting both light and dark modes.
